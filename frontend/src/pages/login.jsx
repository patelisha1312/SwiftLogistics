...
const [suggestions, setSuggestions] = useState([]);
  ...
  // 🔥 Only trigger for email field
  if (name === "email") {
  const list = JSON.parse(localStorage.getItem("rememberList")) || [];
...
const response = await fetch(apiUrl, {
  ...
  // ✅ IMPORTANT: Store token before calling login()
...
const updatedList = [
  ...
  ...existing.filter(item => item.email !== formData.email)
];

localStorage.setItem("rememberList", JSON.stringify(updatedList));
} else {
  localStorage.removeItem("rememberList");
}
  ...
  } else if (location.state?.from === "/schedule-pickup") {
  ...
  window.location.href =
    `https://www.facebook.com/v19.0/dialog/oauth?client_id=${appId}&redirect_uri=https://swiftlogistics-backend.onrender.com/api/auth/facebook/callback&scope=public_profile`;
};
...
/> <span className="ml-2 text-sm text-gray-600">Remember me</span> </label>
<p
              ...
              {role === 'user' && (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <button type="button" onClick={handleFacebookLogin} className="flex items-center justify-center px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors" > <img src="https://www.svgrepo.com/show/475647/facebook-color.svg" alt="Facebook" className="h-5 w-5 mr-2" /> <span className="text-sm font-medium text-gray-700">Facebook</span> </button>
                  </div>
                </>
              )}
        ...
        <div className="hidden lg:block bg-gradient-to-br from-purple-600 to-purple-800 p-12 text-white relative overflow-hidden">
             ...
             <div className="relative z-10 h-full flex flex-col justify-center">
                 ...
                 <div className="space-y-6">
                     ...
                     <div className="flex items-start space-x-4"> <div className="bg-white/20 p-3 rounded-lg"> <MapPin className="h-6 w-6" /> </div> <div> <h4 className="font-semibold text-lg mb-1">Global Coverage</h4> <p className="text-purple-100">Deliver to over 20+ states in India</p> </div> </div>
