...
const submitRating = async () => {
  try {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL}/api/driver/rate-driver`, {
  method: "POST",
  headers: {
...
if (!res.ok) {
  throw new Error(data.msg || "Rating failed");
}
    ...
    setBookings(prev =>
      prev.map(b =>
        b._id === selectedBooking._id ? { ...b, isRated: true } : b
      )
    );
setRating(0);
setComment("");
                                    ...
                                    <div className="p-3 sm:p-4 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => handleToggleDetails(booking.bookingId)}>
                                        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 sm:gap-0">
                                            ...
                                            <div className="flex items-center space-x-3 sm:space-x-4 mt-2 sm:mt-0">
                                                ...
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    navigate(`/track/${booking.trackingId}`);
                                                }}
                                                className="bg-purple-600 text-white px-3 py-1 rounded-lg text-xs sm:text-sm"
                                                >
                                                Track
                                                </button>
                                            ...
                                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                                                ...
                                                <div className="space-y-3 sm:space-y-4 bg-white p-3 sm:p-4 rounded-lg border">
                                                    ...
                                                    <div>
                                                        ...
                                                        {booking.status === "Delivered" && !booking.isRated && (
                                                                <button
                                                        onClick={(e) => {
                                                            e.stopPropagation(); // ✅ IMPORTANT FIX
                                                            openRatingModal(booking);
                                                        }}
                                                        className="mt-3 bg-yellow-500 text-white px-3 py-2 rounded text-sm"
                                                        >
                                                        ⭐ Rate Driver
                                                        </button>
                                                                )}
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
    <div className="bg-white p-6 rounded-lg w-80">
      <h2 className="text-lg font-bold mb-4">Rate Driver</h2>
